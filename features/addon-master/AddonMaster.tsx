...
export function AddonMaster() {
  ...
  return <section className="addon-master" aria-label="Addon sub master">
    ...
    <div className="addon-body">
      ...
      <aside className="addon-help">{helpField === "name" && <><div className="addon-help-head"><span>SELECTED GROUP</span><span>DESCRIPTION</span><span>SHORT</span></div>{groupRecords.map((record) => <button className="addon-help-row" key={record.id} onClick={() => selectRecord(record)}><span>{addonGroups.find((group) => group.id === groupId)?.name}</span><b>{record.name}</b><small>{record.shortName}</small></button>)}<p>Total Help Record : {groupRecords.length}</p></>}{helpField === "state" && <><strong>SELECT STATE</strong>{stateOptions.map((state) => <button className="state-help-row" key={state} onClick={() => { setDraft((current) => ({ ...current, state })); setHelpField(null); }}><b>{state}</b></button>)}</>}</aside>
    </div>
