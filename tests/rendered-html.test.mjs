...
test("keeps the latest migrated application surface wired into the root route", async () => {
  ...
  for (const action of ["Print", "Image", "Log", "module-password", "product-image", "master-log", "zoom-book", "cloud-push", "SCH_SALEHO", "PL_SRATE", "Numeric Filter", "Text Filter", "Date Filter", "Hide / Show Columns", "Clear the ", "\"figure\"", "useDraggable"]) {
    assert.match(master, new RegExp(action));
  }
