...
| SRC-DB-001 | `rishabh_plastic27_backup.sql` | Restricted client data | SHA-256 `ed0124bb02497f397b874370fdbdac9d27febf90f6b677b63585a717398d8d6f`; PostgreSQL custom archive | Restored in isolated local intake database; raw archive/data remain outside Git |
...
| SRC-PROC-001 | `postgres-procedures_validated.sql` and `mssql-all-sp-script.sql` received 2026-08-21 | Confidential business logic | SHA-256, sizes, source/target name coverage, missing routines, and per-routine repair gates recorded in `docs/intake/postgres-procedures-2026-08-21/analysis.json` and `procedure-repair-queue.csv` | Quarantined; 19 of 282 declarations are static compile candidates, 263 require named repairs, none are compile/parity validated, and 46 detected source names are missing |
