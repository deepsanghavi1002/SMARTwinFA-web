...
Their hashes, sizes, name reconciliation, and static conversion defects are
recorded in [analysis.json](analysis.json). Re-run the evidence with:
...
No procedure may advance to converted or parity-tested status until it has a
source/target signature match, resolves its dependencies against a reviewed
schema, loads into disposable PostgreSQL, passes rule/transaction/security
tests, and has representative parity evidence.
